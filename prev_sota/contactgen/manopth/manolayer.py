...
from .tensutils import (th_posemap_axisang, th_with_zeros, subtract_flat_id)
...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        if v_template is None:
            th_v_template = torch.Tensor(smpl_data['v_template'].r).unsqueeze(0)
        else:
            th_v_template = v_template.unsqueeze(0)
        self.register_buffer('th_v_template', th_v_template)
    ...
    def forward(self,
        ...
        if th_v_template is None:
            th_v_template = self.th_v_template
        ...
        if self.use_pca or self.joint_rot_mode == 'axisang':
            ...
            th_full_pose = torch.cat([
                ...
                self.th_hands_mean + th_full_hand_pose], 1)
        ...
        if th_betas is None or th_betas.numel() == 1:
            th_v_shaped = torch.matmul(self.th_shapedirs,
                                       self.th_betas.transpose(1, 0)).permute(
                                           2, 0, 1) + th_v_template
            th_j = torch.matmul(self.th_J_regressor.unsqueeze(0), th_v_shaped)#.repeat( batch_size, 1, 1)

        else:
            th_v_shaped = torch.matmul(self.th_shapedirs,
                                       th_betas.transpose(1, 0)).permute(2, 0, 1) + th_v_template
            th_j = torch.matmul(self.th_J_regressor.unsqueeze(0), th_v_shaped)
        ...
        global_rot = th_results[:, :, :3, :3]
        global_tsl = th_results[:, :, :3, 3:]
        global_tsl = global_tsl + th_trans[:, None, :, None]
        ...
        # th_j = torch.concatenate((th_j[:, reorder_idxs], th_verts[:, tips]), dim=1)

        return th_verts, th_j, global_transf
