...
def run_sim_parallel_interface(params):
...
def run_simulation(
    ...
    client.setPhysicsEngineParameter(numSolverIterations=150)
    client.setPhysicsEngineParameter(fixedTimeStep=simulation_step)
    ...
    hand_collision_id = client.createCollisionShape(
        p.GEOM_MESH,
    ...
    hand_visual_id = client.createVisualShape(
        p.GEOM_MESH,
        fileName=hand_tmp_fname,
        rgbaColor=[0, 0, 1, 1],
        specularColor=[0, 0, 1],
    ...
    hand_body_id = client.createMultiBody(
        baseMass=0,
    ...
    obj_verts_n = obj_verts - obj_center_mass
    # add object
    use_vhacd = True
    # if use_vhacd:
    #     if verbose:
    ...
    #     save_obj(obj_tmp_fname, obj_verts_n, obj_faces)
    #
    #     if not vhacd(obj_tmp_fname, vhacd_exe, resolution=vhacd_resolution):
    #         raise RuntimeError("Cannot compute convex hull " "decomposition for {}".format(obj_tmp_fname))
    #     else:
    #         print(f"Succeeded vhacd decomp of {obj_tmp_fname}")
    #
    #     obj_collision_id = client.createCollisionShape(p.GEOM_MESH, fileName=obj_tmp_fname)
    #     if verbose:
    #         time2 = time.time()
    #         print("Computed v-hacd decomposition at res {} {:.6f} s".format(vhacd_resolution, (time2 - time1)))
    # else:
    scene = trimesh.Scene(obj_hulls)
    scene.export(obj_tmp_fname)
    obj_collision_id = client.createCollisionShape(p.GEOM_MESH, fileName=obj_tmp_fname)

    obj_visual_id = client.createVisualShape(
        p.GEOM_MESH,
    ...
    obj_body_id = client.createMultiBody(
    ...
    for step_idx in range(num_iterations):
        ...
        if save_video:
            img = take_picture(renderer, conn_id=0)
            images.append(img)
        if save_simul_folder:
            ...
            pos, orn = client.getBasePositionAndOrientation(obj_body_id)
            mat = np.reshape(client.getMatrixFromQuaternion(orn), (3, 3))
            obj_verts_t = pos + np.dot(mat, obj_verts_n.T).T
            save_obj(obj_step_path, obj_verts_t, obj_faces)
        time.sleep(wait_time)

    if save_video:
        vid_path = os.path.join(save_video_path, indicator+'.gif')
        write_video(images, vid_path)
        print("Saved gif to {}".format(vid_path))
    pos_end = client.getBasePositionAndOrientation(obj_body_id)[0]
