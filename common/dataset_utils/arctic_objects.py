...
from common.utils.rot import quaternion_apply, axis_angle_to_quaternion
...
class ObjectTensors(nn.Module):
    def __init__(self):
        ...
        self.obj_tensors = construct_obj_tensors(OBJECTS)
    ...
    def to(self, device):
        self.obj_tensors = thing2dev(self.obj_tensors, device)

def construct_obj(object_model_p):
    # load vtemplate
    mesh_p = osp.join(object_model_p, "mesh.obj")
    parts_p = osp.join(object_model_p, f"parts.json")
    json_p = osp.join(object_model_p, "object_params.json")
    obj_name = osp.basename(object_model_p)

    top_sub_p = f"data/arctic/data/arctic_data/data/meta/object_vtemplates/{obj_name}/top_keypoints_300.json"
    ...
    assert osp.exists(mesh_p), f"Not found: {mesh_p}"
    ...
    with open("data/arctic/data/arctic_data/data/meta/object_meta.json", "r") as f:
...
def construct_obj_tensors(object_names):
    ...
    for k in object_names:
        object_model_p = f"data/arctic/data/arctic_data/data/meta/object_vtemplates/%s" % (k)
