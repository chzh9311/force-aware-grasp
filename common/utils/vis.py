...
def get_bimanip_meshes(lh_vs: torch.Tensor, lh_f: np.ndarray, rh_vs: torch.Tensor, rh_f: np.ndarray,
                       obj_vs: list, obj_fs: list):
    lh_vs, rh_vs = lh_vs.cpu().numpy(), rh_vs.cpu().numpy()
    lh_meshes = [o3dmesh(lh_vs[i], lh_f, color=(np.array([0.2, 0.3, 0.8]))) for i in range(lh_vs.shape[0])]
    rh_meshes = [o3dmesh(rh_vs[i], rh_f, color=(np.array([0.8, 0.4, 0.2]))) for i in range(lh_vs.shape[0])]
    obj_meshes = [o3dmesh(obj_vs[i], obj_fs[i], color=(np.array([0.5, 0.6, 0.4]))) for i in range(len(obj_vs))]
    return lh_meshes, rh_meshes, obj_meshes

def vis_bimanip_batch(lh_vs: torch.Tensor, lh_f: np.ndarray,
                      rh_vs: torch.Tensor, rh_f: np.ndarray,
                      obj_vs: list, obj_fs: list, sampled_pts: (np.ndarray, None)=None,
                      contacts: (np.ndarray, None)=None, labels: (list, None)=None, window_name: str="Hands&Obj"):
    lh_meshes, rh_meshes, obj_meshes = get_bimanip_meshes(lh_vs, lh_f, rh_vs, rh_f, obj_vs, obj_fs)
    hand_mat = vis.rendering.MaterialRecord()
    hand_mat.shader = "defaultLitTransparency"
    hand_mat.base_color = [0.8, 0.7, 0.5, 0.4]
    obj_mat = vis.rendering.MaterialRecord()
    obj_mat.shader = "defaultLitTransparency"
    obj_mat.base_color = [0.5, 0.5, 0.5, 0.9]
    n_samples = len(lh_meshes)
    all_pts = np.concatenate(obj_vs, axis=0)
    scale = np.mean(np.max(all_pts, axis=0) - np.min(all_pts, axis=0))
    if sampled_pts is not None:
        obj_pcs = []
        for i in range(n_samples):
            obj_pc = o3d.geometry.PointCloud()
            obj_pc.points = o3d.utility.Vector3dVector(sampled_pts[i])
            colors = np.zeros_like(sampled_pts[i])
            colors[:, 0] = contacts[i]
            obj_pc.colors = o3d.utility.Vector3dVector(colors)
            obj_pcs.append(obj_pc)

    # Visualize them in one image
    grid_w = np.ceil(np.sqrt(n_samples))
    vis_list = []
    for i in range(n_samples):
        lh_mesh, rh_mesh, obj_mesh = lh_meshes[i], rh_meshes[i], obj_meshes[i]
        center = obj_mesh.get_center().reshape(3)
        iw, ih = int(i % grid_w), int(i // grid_w)
        base_pos = np.array([iw+0.5, ih+0.5, 0]) * 2 * scale
        pos = base_pos - center
        lh_mesh.translate(pos)
        rh_mesh.translate(pos)
        obj_mesh.translate(pos)
        vis_list.extend([{'name': f'lh_{i:d}', 'geometry': lh_mesh, 'material': hand_mat},
                         {'name': f'rh_{i:d}', 'geometry': rh_mesh, 'material': hand_mat},
                         {'name': f'obj_{i:d}', 'geometry': obj_mesh, 'material': obj_mat}])
        if sampled_pts is not None:
            obj_pc = obj_pcs[i]
            obj_pc.translate(pos)
            vis_list.append(obj_pc)
        if labels is not None:
            label = f"{labels[i]:.2f}"
            label_txt = o3d.t.geometry.TriangleMesh.create_text(text=label, depth=0.05).to_legacy()
            label_txt.scale(0.01, center=(0, 0, 0))
            label_txt.paint_uniform_color((0, 0, 0))
            label_txt.translate(base_pos + np.array([0, 0.3*scale, 0.1*scale]))
            vis_list.append({'name': f'score_{i:d}', 'geometry': label_txt, 'material': obj_mat})

    vis_list.append(o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1))

    o3d.visualization.draw(vis_list)



def vis_bimanip_seq(lh_mano_array: np.ndarray, rh_mano_array: np.ndarray,
                    mano_layers: dict, objs_v0: list, objs_f: list, objs_traj: list, window_name="Hands&Obj"):
    """
    lh/rh_mano_array: [N_f, 61], including global translation (3) + global rotation (3) + mano pose (45) + mano shape(10);
    objs_v0: list of [N_f, N_v, 3], the object vertices at t=0;
    objs_f: list of [N_f, N_face, 3], the object faces;
    objs_traj: list of [N_f, 4, 4], the object trajectory;
    """
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name)
    vis.clear_geometries()
    lh_mano_array = torch.from_numpy(lh_mano_array).float()
    rh_mano_array = torch.from_numpy(rh_mano_array).float()
    lh_dict= {'trans': lh_mano_array[:, :3], 'rot_aa': lh_mano_array[:, 3:6], 'pose': lh_mano_array[:, 6:51],
              'shape': lh_mano_array[:, 51:]}
    rh_dict= {'trans': rh_mano_array[:, :3], 'rot_aa': rh_mano_array[:, 3:6], 'pose': rh_mano_array[:, 6:51],
               'shape': rh_mano_array[:, 51:]}
    # lh_vs, lh_f = mano_layers.mesh_data_np(lh_dict, is_right=False)
    # rh_vs, rh_f = mano_layers.mesh_data_np(rh_dict, is_right=True)
    lh = mano_layers['left'](lh_mano_array[:, 3:51], lh_dict['shape'])
    rh = mano_layers['right'](rh_mano_array[:, 3:51], rh_dict['shape'])
    lh_vs, rh_vs = lh[0] + lh_dict['trans'].reshape(-1, 1, 3), rh[0] + rh_dict['trans'].reshape(-1, 1, 3)
    lh_mesh = o3dmesh(lh_vs[0], mano_layers['lf'])
    rh_mesh = o3dmesh(rh_vs[0], mano_layers['rf'])
    vis.add_geometry(lh_mesh)
    vis.add_geometry(rh_mesh)
    vis.add_geometry(o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.05))
    obj_meshes = []
    for i in range(len(objs_v0)):
        obj_meshes.append(o3dmesh(objs_v0[i], objs_f[i]))
        vis.add_geometry(obj_meshes[-1])
    obj_meshes_t0 = copy.deepcopy(obj_meshes)
    for t in range(lh_mano_array.shape[0]):
        lh_mesh.vertices = o3d.utility.Vector3dVector(lh_vs[t])
        vis.update_geometry(lh_mesh)
        rh_mesh.vertices = o3d.utility.Vector3dVector(rh_vs[t])
        vis.update_geometry(rh_mesh)
        ## Objects
        for i in range(len(objs_f)):
            obj_meshes[i].vertices = obj_meshes_t0[i].vertices
            obj_meshes[i].transform(objs_traj[i][t])
            vis.update_geometry(obj_meshes[i])
        vis.poll_events()
        vis.update_renderer()

    vis.close()

def vis_mesh_seq(vert_list, faces, window_name):
    """
    vert_list: the list of vertice_seqs of (L_seq x N_vert x 3)
    faces: the list of faces of (N_faces x 3)
    """
    assert len(faces) == len(vert_list)
    n_vis_mesh = len(vert_list)
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name)
    ## Init
    meshes = []
    vis.clear_geometries()
    for i in range(n_vis_mesh):
        verts = o3d.utility.Vector3dVector(vert_list[i][0])
        faces = o3d.utility.Vector3iVector(faces[i])
        meshes.append(o3d.geometry.TriangleMesh(vertices=verts, faces=faces))
        vis.add_geometry(meshes[-1])

    for t in range(len(vert_list[0].shape[0])):
        for i in range(n_vis_mesh):
            meshes[i].vertices = o3d.utility.Vector3dVector(vert_list[i][t])
            vis.update_geometry(meshes[i])
            vis.poll_events()
            vis.update_renderer()


def o3d_quiver(anchors: np.ndarray, directions: np.ndarray,
               colors:(np.ndarray, None)=None, scale: float=1.0) -> list:
    n = anchors.shape[0]
    arrows = [o3d_arrow(anchors[i], directions[i], colors[i], scale) for i in range(n)]
...
class AppWindow:
    ...
    def __init__(self, width: int, height: int, templates: dict, vis_seqs: list):
        """
        templates: a dict of meshes to initialize ('name': mesh)
        vis_seqs: a list of list of dictionaries of meshes to visualize, with keys 'name', 'vertices', 'color'. 'vertices' is in
        o3d.utility.Vector3dVector format. Static meshes can be shared throughout all frames, and only colors are changed.
        """
        ...
        self.param_control = gui.CollapsableVert("Parameters Control", 0, gui.Margins(em, 0, 0, 0))

        # initial slider:
        n_frames = len(vis_seqs)
        self.frame_slider = gui.Slider(gui.Slider.INT)
        self.frame_slider.set_limits(0, n_frames - 1)
        self.frame_slider.set_on_value_changed(self._change_vis_frame)

        self.param_control.add_child(self.frame_slider)

        self._settings_panel.add_child(self.param_control)
        ...
        self.vis_seqs = vis_seqs
        self.templates = templates
        self.vis_frame = 0

        ## initialize
        self._scene.scene.clear_geometry()
        for mname, mv in templates.items():
            try:
                # mesh_set_color(mesh['color'], mesh['mesh'])
                self._scene.scene.add_geometry(mname, mv, rendering.MaterialRecord())
            except Exception as e:
                print(e)
        self._update_geometry()
        # for mp in vis_seqs[0]:
        #     mesh = self.templates[mp['name']]
        #     if mp['vertices'] is not None:
        #         mesh.vertices = mp['vertices']
        #     if mp['color'] is not None:
        #         mesh_set_color(mp['color'], mesh)
        #     self._scene.scene.update_geometry(mp['name'], mesh)
        bounds = self._scene.scene.bounding_box
        self._scene.setup_camera(60, bounds, bounds.get_center())
        # self.template = template_list[0]["template"]
        # betas = np.random.randn(MANO_Api.NUM_BETAS)
        # thetas = np.random.randn(MANO_Api.NUM_THETAS)
        # self.mesh = self.template.calculate_shape(self.betas, self.thetas)
        # indices = o3d.utility.Vector3iVector(self.template.face_idx)
        # self.mesh = o3d.geometry.TriangleMesh()
        # self.mesh.triangles = indices
        # vertices = o3d.utility.Vector3dVector(self.template.calculate_shape(self.betas, self.thetas))
    ...
    def _change_vis_frame(self, val):
        new_value = int(val)
        if self.vis_frame != new_value:
            self.vis_frame = new_value
            self._update_geometry()
    ...
    def _update_geometry(self):
        mesh_list = self.vis_seqs[self.vis_frame]
        for mp in mesh_list:
            mesh = self.templates[mp['name']]
            if mp['vertices'] is not None:
                mesh.vertices = mp['vertices']
            if mp['color'] is not None:
                mesh_set_color(mp['color'], mesh)
            self._scene.scene.remove_geometry(mp['name'])
            self._scene.scene.add_geometry(mp['name'], mesh, rendering.MaterialRecord())
        self._scene.scene.update_material(self.settings.material)