...
def approx_sdf(mesh_verts, mesh_normals, query_points, query_normals, mesh_trimesh, query_trimesh, caps_rad, foreach_on_mesh):
    if foreach_on_mesh:
        knn_dists, nearest_idx, nearest_pos = normal_matched_nn(mesh_verts, mesh_normals, query_points, query_normals, mesh_trimesh, query_trimesh, True)
        knn_dists, nearest_idx, nearest_pos = knn_dists.unsqueeze(2), nearest_idx.unsqueeze(2), nearest_pos.unsqueeze(2)
    else:
        knn_dists, nearest_idx, nearest_pos = normal_matched_nn(query_points, query_normals, mesh_verts, mesh_normals, mesh_trimesh, query_trimesh, True)
        knn_dists, nearest_idx, nearest_pos = knn_dists.unsqueeze(2), nearest_idx.unsqueeze(2), nearest_pos.unsqueeze(2)

    return knn_dists / caps_rad, nearest_idx

def hosdf2contact(sd, out_tolerance, in_tolerance):
    outside = sd < -out_tolerance
    inside = sd > in_tolerance
    c = torch.ones_like(sd, device=sd.device)
    c[outside] = 1 / (- sd[outside] - out_tolerance + 1)
    c[inside] = 1 / (sd[inside] - in_tolerance + 1)

    return c

def calculate_contact_signed_dist(hand_verts, hand_normals, object_verts, object_normals, object_meshes=None, hand_meshes=None, caps_top=0.0005, caps_bot=-0.0015,
                                  caps_rad=0.001):
    sdf_obj, nn_idx = approx_sdf(object_verts, object_normals, hand_verts, hand_normals, object_meshes, hand_meshes, caps_rad, True)
    sdf_hand, _ = approx_sdf(object_verts, object_normals, hand_verts, hand_normals, object_meshes, hand_meshes, caps_rad, False)

    obj_contact = hosdf2contact(sdf_obj, abs(caps_top) / caps_rad, abs(caps_bot) / caps_rad)# * (dot_obj/2+0.5) # TODO dotting contact normal
    hand_contact = hosdf2contact(sdf_hand, abs(caps_top) / caps_rad, abs(caps_bot) / caps_rad)# * (dot_hand/2+0.5)

    # print('oshape, nshape', obj_contact.shape, (dot_obj/2+0.5).shape)##

    return obj_contact, hand_contact, sdf_obj, nn_idx

def capsule_sdf(mesh_verts, mesh_normals, query_points, query_normals, caps_rad, caps_top, caps_bot, foreach_on_mesh, fix_normal=False):
    """
    ...
    if foreach_on_mesh:     # Foreach mesh vert, find closest query point
        if fix_normal:
            knn_dists, nearest_idx, nearest_pos = normal_matched_nn(mesh_verts, mesh_normals, query_points, query_normals, signed_dist=False)
            knn_dists, nearest_idx, nearest_pos = knn_dists.unsqueeze(2), nearest_idx.unsqueeze(2), nearest_pos.unsqueeze(2)
        else:
        ...
        if fix_normal:
            knn_dists, nearest_idx, nearest_pos = normal_matched_nn(query_points, query_normals, mesh_verts, mesh_normals, signed_dist=False)
            knn_dists, nearest_idx, nearest_pos = knn_dists.unsqueeze(2), nearest_idx.unsqueeze(2), nearest_pos.unsqueeze(2)
        else:
...
def calculate_part_contact_capsule(hand_verts, hand_normals, object_verts, object_normals, obj_part_mask, hand_part_id,
                              caps_top=0.0005, caps_bot=-0.0015, caps_rad=0.001, caps_on_hand=False, contact_norm_method=0, fix_normal=False):
    """
    ...
    """
    obj_contact = torch.zeros(object_verts.shape[:2], device=object_verts.device)
    for i in range(16):
        hand_mask = hand_part_id == i
        # if caps_on_hand:
        #     sdf_obj, dot_obj, nn_idx = capsule_sdf(hand_verts[hand_mask], hand_normals[hand_mask], object_verts[obj_mask],
        #                                            object_normals[obj_mask], caps_rad, caps_top, caps_bot, False, fix_normal)
        #     sdf_hand, dot_hand, _ = capsule_sdf(hand_verts[hand_mask], hand_normals[hand_mask], object_verts[obj_mask],
        #                                         object_normals[obj_mask], caps_rad, caps_top, caps_bot, True, fix_normal)
        sdf_obj, dot_obj, nn_idx = capsule_sdf(object_verts, object_normals, hand_verts[:, hand_mask],
                                               hand_normals[:, hand_mask], caps_rad, caps_top, caps_bot, True, fix_normal)
        # sdf_hand, dot_hand, _ = capsule_sdf(object_verts[obj_mask], object_normals[obj_mask], hand_verts[hand_mask],
        #                                     hand_normals[hand_mask], caps_rad, caps_top, caps_bot, False, fix_normal)

        obj_contact[obj_part_mask[:, i]] = sdf_to_contact(sdf_obj, dot_obj, method=contact_norm_method)[obj_part_mask[:, i]]# * (dot_obj/2+0.5)
        # hand_contact = sdf_to_contact(sdf_hand, dot_hand, method=contact_norm_method)# * (dot_hand/2+0.5)

    # print('oshape, nshape', obj_contact.shape, (dot_obj/2+0.5).shape)##

    return obj_contact.unsqueeze(2)
...
def calculate_contact_capsule(hand_verts, hand_normals, object_verts, object_normals,
    ...
    """
    if caps_on_hand:
        sdf_obj, dot_obj, nn_idx = capsule_sdf(hand_verts, hand_normals, object_verts, object_normals, caps_rad, caps_top, caps_bot, False, fix_normal)
        sdf_hand, dot_hand, _ = capsule_sdf(hand_verts, hand_normals, object_verts, object_normals, caps_rad, caps_top, caps_bot, True, fix_normal)
    else:
        sdf_obj, dot_obj, nn_idx = capsule_sdf(object_verts, object_normals, hand_verts, hand_normals, caps_rad, caps_top, caps_bot, True, fix_normal)
        sdf_hand, dot_hand, _ = capsule_sdf(object_verts, object_normals, hand_verts, hand_normals, caps_rad, caps_top, caps_bot, False, fix_normal)

    obj_contact = sdf_to_contact(sdf_obj, dot_obj, method=contact_norm_method)# * (dot_obj/2+0.5)
    hand_contact = sdf_to_contact(sdf_hand, dot_hand, method=contact_norm_method)# * (dot_hand/2+0.5)

    # print('oshape, nshape', obj_contact.shape, (dot_obj/2+0.5).shape)##

    return obj_contact.unsqueeze(2), hand_contact.unsqueeze(2), sdf_obj, sdf_hand, nn_idx
