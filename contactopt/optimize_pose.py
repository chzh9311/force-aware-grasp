...
def optimize_pose(data, hand_contact_target, obj_contact_target, n_iter=250, lr=0.01, w_cont_hand=2, w_cont_obj=1,
                  ...
                  contact_norm_method=0, w_pen_cost=600, w_obj_rot=0, pen_it=0, physics_solver=None, coms=None,
                  object_meshes=None, hand_meshes=None, contact_weights=None):
    ...
    mano_model = ManoLayer(mano_root='data/mano_v1_2/models', use_pca=True, ncomps=ncomps, side='right', flat_hand_mean=False).to(device)
    ...
    opt_state = []
    is_thin = util.mesh_is_thin(data['mesh_aug'].num_verts_per_mesh())
    ...
    for it in range(n_iter):
        ...
        contact_obj, contact_hand, nn_idx = calculate_contact.calculate_contact_capsule(hand_verts, hand_normals, obj_verts, obj_normals,
                              caps_top=caps_top, caps_bot=caps_bot, caps_rad=caps_rad, caps_on_hand=caps_on_hand, contact_norm_method=contact_norm_method,
                                                                                        # fix_normal=physics_solver is not None)
                                                                                        fix_normal=False)

        if contact_weights is not None:
            obj_contact_weights = linear_normalize(contact_weights.unsqueeze(-1), 0.3, 1)
            contact_obj_sub = obj_contact_weights * (obj_contact_target - contact_obj)
        else:
