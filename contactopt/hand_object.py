...
class HandObject:
    ...
    def calc_dist_contact(self, hand=True, obj=False, special_contact=False):
        ...
        if not special_contact:
            obj_contact, hand_contact, _ = calculate_contact_capsule(hand_verts, hand_mesh.verts_normals_padded(), object_mesh.verts_padded(), object_mesh.verts_normals_padded())
        else:
            ...
            obj_contact, hand_contact, _ = calculate_contact_capsule(hand_verts_subdivided, hand_normals_subdivided, object_mesh.verts_padded(),
    ...
    def run_mano(self):
        ...
        if self.hand_pose.shape[0] == 48:   # Special case when we're loading GT honnotate
            mano_model = ManoLayer(mano_root='data/mano_v1_2/models', joint_rot_mode="axisang", use_pca=False, center_idx=None, flat_hand_mean=True)
